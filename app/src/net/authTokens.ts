// Token-bundle helpers for the SSO-hash seeding in session.ts.
//
// mero-js stores auth as a single JSON blob at `mero-tokens` (its
// LocalStorageTokenStore); anything that rotates the access/refresh pair reads
// and writes that same blob. These helpers decide whether the bundle carried in
// an SSO / auth-callback hash should replace what is already stored.
 ...
 *
 * This is the same source mero-react/mero-js use, so a stored bundle and a hash
 * bundle are always compared on the same scale. Returns null for a token that
 * isn't a decodable JWT (e.g. an opaque dev token).
 */
 ...
 * `/auth/refresh` consumes the presented refresh token and mints a new one. The
 * desktop re-opens this app with the bundle it minted at *its* login, so the
 * hash routinely carries a bundle OLDER than the stored one — mero-js may have
 * rotated since. Overwriting the store with the hash would re-present an already
 ...
 * different node (foreign token family), or a genuinely newer hash bundle (a
 * fresh login).
 */
