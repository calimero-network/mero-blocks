...
const store = (tokens: { access_token: string; refresh_token: string; expires_at: number }) =>
...
describe("tokenExpiryMs", () => {
  it("prefers the JWT `exp` over the recorded expires_at", () => {
    ...
    expect(tokenExpiryMs({ access_token: jwt(exp), refresh_token: "rt", expires_at: 1 })).toBe(
      Math.floor(exp / 1000) * 1000,
    );
  });

  it("falls back to expires_at for opaque tokens", () => {
    expect(tokenExpiryMs({ access_token: "opaque", refresh_token: "rt", expires_at: 77 })).toBe(77);
  });
});
...
describe("shouldSeedTokens (single-use refresh — core#3083)", () => {
  ...
  it("seeds when nothing is stored", () => {
    expect(shouldSeedTokens({ hashExpiresAtMs: now + 60_000, stored: null, nodeChanged: false })).toBe(
      true,
    );
  });

  it("does NOT clobber a bundle already rotated past the hash", () => {
    // The desktop re-opens us with the bundle it minted at ITS login, while a
    // newer bundle has since been rotated into this browser. The stored refresh
    // token is the only live one; the hash's was consumed by that rotation.
    // Re-presenting it would be read as theft (401 `token_reuse`).
    expect(
      shouldSeedTokens({
