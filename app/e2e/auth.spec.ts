...
const storedSession = (page: import("@playwright/test").Page) =>
  page.evaluate(() => JSON.parse(localStorage.getItem("mb-session") ?? "null"));
...
test.describe("web login", () => {
  ...
  test("disconnecting drops the token bundle, not just the world", async ({ page }) => {
    ...
    expect(await page.evaluate(() => localStorage.getItem("mb-session"))).toBeNull();
  });
});
...
test.describe("creating a world", () => {
  ...
  test("a blank name becomes 'overworld', and the name is remembered locally", async ({ page }) => {
    const captured: CapturedBodies = {};
    await seedAuthOnly(page);
    await mockNode(page, freshState());
    await mockAdmin(page, [], captured);
    ...
    expect(captured.namespace?.name).toBe("overworld");
    expect(captured.group).toEqual({ groupName: "overworld", visibility: "open" });
    ...
    expect(names["ctx-created"]).toBe("overworld");
  });
